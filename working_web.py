#!/usr/bin/env python3
"""
Working PatchWatch Web Interface
Simple web interface that actually works
"""
...
<body>
    <div class="container">
        ...
        <div class="info-box">
            <h3>🌐 GitLab Repository Configuration</h3>
    ...
    <script>
        ...
        async function testPath() {{
            ...
            try {{
                ...
                if (result.accessible) {{
                    showAlert(`✅ Path is accessible! Response time: ${{Math.round(result.response_time * 1000)}}ms`, 'success');
                }} else {{
                    showAlert(`❌ Path not accessible: ${{result.error_details}}`, 'error');
                }}
            }} catch (error) {{
        ...
        async function saveConfig() {{
            ...
            try {{
                const response = await fetch('/save-config', {{
                    ...
                    headers: {{'Content-Type': 'application/json'}},
                    body: JSON.stringify({{
                        local_developer_folder: path,
                        path_type: pathType
                    }})
                }});
                
                if (response.ok) {{
                    showAlert('✅ Path configuration saved successfully!', 'success');
                    document.getElementById('currentFolder').textContent = path;
                }} else {{
                    showAlert('❌ Error saving path configuration', 'error');
                }}
            }} catch (error) {{
        ...
        async function saveGitLabConfig() {{
            ...
            if (!gitlabUrl.trim() || !gitlabToken.trim()) {{
                showAlert('Please enter GitLab URL and token', 'error');
                return;
            }}
            ...
            try {{
                ...
                if (response.ok) {{
                    ...
                    // Update the GitLab Source of Truth link
                    const gitlabLink = document.querySelector('a[href*="gitlab"]');
                    if (gitlabLink) {{
                        gitlabLink.href = gitlabUrl;
                        gitlabLink.textContent = gitlabUrl;
                    }}
                }} else {{
                    showAlert('❌ Error saving GitLab configuration', 'error');
                }}
            }} catch (error) {{
        ...
        async function testGitLabConnection() {{
            ...
            if (!gitlabUrl.trim() || !gitlabToken.trim()) {{
                showAlert('Please enter GitLab URL and token first', 'error');
                return;
            }}
            
            try {{
                showAlert('🔗 Testing GitLab connection...', 'info');
                ...
                if (response.ok) {{
                    const result = await response.json();
                    if (result.success) {{
                        showAlert(`✅ GitLab connection successful! User: ${{result.user_name}} (Access level: ${{result.access_level}})`, 'success');
                    }} else {{
                        showAlert(`❌ GitLab connection failed: ${{result.error}}`, 'error');
                    }}
                }} else {{
                    showAlert('❌ Error testing GitLab connection', 'error');
                }}
            }} catch (error) {{
        ...
        async function loadStatus() {{
            try {{
                ...
                const statusClass = data.local_path_accessible ? 'status-ok' : 'status-error';
                const statusText = data.local_path_accessible ? 'Path is accessible' : 'Path is not accessible';
                
                statusArea.innerHTML = `<div class="status ${{statusClass}}">Status: ${{statusText}}</div>`;
        ...
        async function startMonitoring() {{
            try {{
                ...
                if (response.ok) {{
                    ...
                    updateMonitoringStatus({{'active': true, 'status': 'running'}});
                }} else {{
        ...
        async function stopMonitoring() {{
            try {{
                ...
                if (response.ok) {{
                    ...
                    updateMonitoringStatus({{'active': false, 'status': 'stopped'}});
                }} else {{
        ...
        async function fullScan() {{
            try {{
                showAlert('🔍 Начинаю полное сканирование папки...', 'info');
...
def main():
    ...
    config = load_config()
    gitlab_web_url = config['gitlab_url'].rstrip('/') + '/-/tree/main/data'
    
    print("🚀 Starting PatchWatch Working Web Interface...")
    print(f"📁 GitLab Source of Truth: {gitlab_web_url}")
    print(f"👤 Git Author: {config['git_author_name']} <{config['git_author_email']}>")
